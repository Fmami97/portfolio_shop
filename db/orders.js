...
//used for display in the desired order details
exports.getOrderByIds = async (user_id, order_id) => {
    const result = await pool.query('SELECT * FROM orders WHERE user_id = $1 AND id = $2',
        [user_id, order_id]);
    return result.rows;
}
...
//using the method calculateOrderTotalPrice  (done in cartsRouter.js)
exports.updateOrderTotalPrice = async (order_id, total_price) => {
    const result = await pool.query('UPDATE orders SET total_price = $1 WHERE id = $2 RETURNING *',
